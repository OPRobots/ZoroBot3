...
class SlalomTurnProfile(TurnProfile):
    ...
    def describe(self):
        ...
        print('Linear velocity: %.3f' % self.profile['linear_velocity'].iloc[0])
        print('Max angular acceleration: %.3f' % self.profile['max_angular_acceleration'].iloc[0])
        print('Finished at (%.5f, %.5f)' % tuple(self.profile[['x', 'y']].iloc[-1].values))
        print('Parameters: {\n'
        '\t.start = %.5f,\n'
        '\t.end = %.5f,\n'
        '\t.linear_speed = %.5f,\n'
        '\t.max_angular_speed = %.5f,\n'
        '\t.transition = %.5f,\n'
        '\t.arc = %.5f,\n'
        '\t.sign = ??,\n'
        '}' % (
          ...
          self.profile['linear_velocity'].iloc[0]/radius,
          transition,
          arc))
    ...
    def describe_profile(self):
        ...
        desc = ('{\n'
          ...
          '\t},' % (
            before*1000.,
            after*1000.,
            self.profile['linear_velocity'].iloc[0]*1000,
            self.profile['linear_velocity'].iloc[0]/radius,
            transition*1000,
            arc*1000))
        return desc
    ...
    def plot_trajectory(self, title = ""):
        figure, axes = pyplot.subplots(
            1, figsize=(16,16), subplot_kw={'aspect': 'equal'})
        post = self.maze.post
        axes.add_patch(Rectangle((0, 0), 0.18, -0.18, linewidth=1, edgecolor='m', facecolor='gray', alpha=0.3))
        axes.add_patch(Rectangle((0, 0), -0.18, -0.18, linewidth=1, edgecolor='m', facecolor='gray', alpha=0.3))
        axes.add_patch(Rectangle((0, 0), -0.18, 0.18, linewidth=1, edgecolor='m', facecolor='gray', alpha=0.3))
        axes.add_patch(Rectangle((0, 0), 0.18, 0.18, linewidth=1, edgecolor='m', facecolor='gray', alpha=0.3))

        axes.add_patch(Rectangle((-post / 2, -post / 2), post, post))
        axes.add_patch(Rectangle((-post / 2 - 0.18, -post / 2 + 0.18), post, post))
        axes.add_patch(Rectangle((-post / 2, -post / 2 + 0.18), post, post))
        axes.add_patch(Rectangle((-post / 2 + 0.18, -post / 2 + 0.18), post, post))
        axes.add_patch(Rectangle((-post / 2 - 0.18, -post / 2), post, post))
        axes.add_patch(Rectangle((-post / 2 + 0.18, -post / 2), post, post))
        axes.add_patch(Rectangle((-post / 2 - 0.18, -post / 2 - 0.18), post, post))
        axes.add_patch(Rectangle((-post / 2, -post / 2 - 0.18), post, post))
        axes.add_patch(Rectangle((-post / 2 + 0.18, -post / 2 - 0.18), post, post))
        ...
        ax.grid(True, which='minor', linestyle='-', linewidth=1.2)
...
def turn_profile(angle, max_angular_velocity,
    ...
    profile = DataFrame({'angular_velocity': angular_velocity, 'max_angular_acceleration': max_angular_acceleration})
...
class Simulator:
    ...
    def slalom(self, entry, exit, radius, linear_velocity=0.5, shift=None):
        x, y, angle = entry
        ...
        force = (linear_velocity**2 * self.robot.mass) / (2 * radius)
