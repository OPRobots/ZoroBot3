...
/**
 * Table to calculate the ln of values between `1` and `ADC_RESOLUTION - 1`.
 *
 * @note It uses steps of size `LOG_CONVERSION_TABLE_STEP`.
 * @note To avoid unstable results, it calculates `ln(0)` as `1`.
 */
...
/**
 * @brief Apply `ln()` to the raw sensor readings.
 *
 * A minimum difference of `1` is enforced to avoid applying `ln()` to a zero
 * or negative value.
 *
